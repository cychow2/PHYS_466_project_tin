...
for timestep in timestep_list:
    ...
    ax = fig.gca()
    ax.set_title(rf"Energy per atom, T={temperature}")
    ax.set_xlabel("step")
    ax.set_ylabel("energy (eV)")
    ax.plot(range(0, nstep+1, 100), energies, label=f"timestep = {timestep}")
    ax.ticklabel_format(useOffset=False, style='plain')
    ax.legend()
    fig.savefig(f"energies_vs_step_tstep={timestep}.png")